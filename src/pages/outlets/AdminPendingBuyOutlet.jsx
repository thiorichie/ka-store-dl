...
function AdminPendingBuyOutlet() {
  const [orders, setOrders] = useState([]);
  const [searchTerm, setSearchTerm] = useState('');

  const fetchData = async () => {
    try {
      const response = await axios.get('https://ka-store-backend-production.up.railway.app/api/buyDL/getTransactions');
  ...
  const handleCancelOrder = async (transactionId) => {
    try {
      await axios.put(`https://ka-store-backend-production.up.railway.app/api/buyDL/cancelTransaction/${transactionId}`);
  ...
  const handleCompleteOrder = async (transactionId) => {
    try {
      await axios.put(`https://ka-store-backend-production.up.railway.app/api/buyDL/completeTransaction/${transactionId}`);
      setOrders(orders.filter((order) => order.transactionId !== transactionId));
  ...
  return (
    <div style={styles.container}>
      <h1 style={styles.title}>Pending Orders</h1>
      <input
        type="text"
        ...
        {filteredOrders.map((order) => (
          <div key={order.transactionId} style={styles.orderCard}>
            <div style={styles.orderInfo}>
              <p><strong>Jenis Transaksi:</strong> Pembelian DL</p>
              <p><strong>Nomor Transaksi:</strong> {order.transactionId}</p>
              <p><strong>Nama World:</strong> {order.namaWorld}</p>
              ...
              <p><strong>Jumlah Dana yang Dibayarkan: </strong> Rp. {order.totalPrice}</p>
...
export default AdminPendingBuyOutlet;